...
const PromotionRunTimeOutLimit = 10
...
func (r *ApplicationPromotionRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := checkForExistingActivePromotions(ctx, *promotionRun, r.Client); err != nil {
		...
		if _, err = updateStatusConditions(ctx, r.Client, "Error occurred while checking for existing active promotions.", promotionRun, appstudioshared.PromotionRunConditionErrorOccurred,
	...
	if promotionRun.Spec.AutomatedPromotion.InitialEnvironment != "" {
		...
		// Update Status.Conditions field.
		if _, err := updateStatusConditions(ctx, r.Client, "Automated promotion are not yet supported.", promotionRun, appstudioshared.PromotionRunConditionErrorOccurred,
			appstudioshared.PromotionRunConditionStatusFalse, appstudioshared.PromotionRunReasonErrorOccurred); err != nil {
			return ctrl.Result{}, fmt.Errorf("unable to update promotionRun %v", err)
		}
		return ctrl.Result{}, nil
	...
	if promotionRun.Spec.ManualPromotion.TargetEnvironment == "" {
		...
		// Update Status.Conditions field.
		if _, err := updateStatusConditions(ctx, r.Client, "Target Environment has invalid value.", promotionRun, appstudioshared.PromotionRunConditionErrorOccurred,
			appstudioshared.PromotionRunConditionStatusFalse, appstudioshared.PromotionRunReasonErrorOccurred); err != nil {
			return ctrl.Result{}, fmt.Errorf("unable to update promotionRun %v", err)
		}
		return ctrl.Result{}, nil
	...
	if len(promotionRun.Status.ActiveBindings) > 0 {
		for _, existingActiveBinding := range promotionRun.Status.ActiveBindings {
			...
			if existingActiveBinding != binding.Name {
				...
				// Update Status.Conditions field.
				if _, err = updateStatusConditions(ctx, r.Client, message, promotionRun, appstudioshared.PromotionRunConditionErrorOccurred,
					appstudioshared.PromotionRunConditionStatusFalse, appstudioshared.PromotionRunReasonErrorOccurred); err != nil {
					return ctrl.Result{}, fmt.Errorf("unable to update promotionRun %v", err)
				}
				return ctrl.Result{}, nil
	...
	if err := r.Client.Get(ctx, client.ObjectKeyFromObject(&applicationSnapshot), &applicationSnapshot); err != nil {
		if apierr.IsNotFound(err) {
			log.Error(err, "Snapshot: "+applicationSnapshot.Name+" referred in Binding: "+binding.Name+" does not exist.")
			return ctrl.Result{}, fmt.Errorf("Snapshot: " + applicationSnapshot.Name + " referred in Binding: " + binding.Name + " does not exist.")
		} else {
			log.Error(err, "unable to retrieve ApplicationSnapshot: "+applicationSnapshot.Name)
	...
	if len(binding.Status.GitOpsDeployments) != len(binding.Spec.Components) {
		...
		if _, err = updateStatusEnvironmentStatus(ctx, r.Client, "Waiting for the environment binding to create all of the expected GitOpsDeployments.",
	...
	if !promotionRun.Status.PromotionStartTime.IsZero() && (metav1.Now().Sub(promotionRun.Status.PromotionStartTime.Time).Minutes() > PromotionRunTimeOutLimit) {
		...
		// Update Status.Environment.Status field.
		if _, err = updateStatusEnvironmentStatus(ctx, r.Client, "Promotion Failed. Could not be completed in "+strconv.Itoa(PromotionRunTimeOutLimit)+" Minutes.",
	...
	if len(waitingGitOpsDeployments) > 0 {
		...
		// Update Status.Environment.Status field.
		if _, err = updateStatusEnvironmentStatus(ctx, r.Client, "Waiting for following GitOpsDeployments to be Synced/Healthy: "+strings.Join(waitingGitOpsDeployments[:], ", "),
			promotionRun, appstudioshared.ApplicationPromotionRunEnvironmentStatus_InProgress); err != nil {
			return ctrl.Result{}, fmt.Errorf("unable to update promotionRun %v", err)
		}
	...
	// Update Status.Environment.Status field.
	if _, err = updateStatusEnvironmentStatus(ctx, r.Client, "All GitOpsDeployments are Synced/Healthy",
...
func updateStatusConditions(ctx context.Context, client client.Client, message string,
	...
	// Check if condition with same message is already set, if Yes then update existing element else append new element.
	index := -1
