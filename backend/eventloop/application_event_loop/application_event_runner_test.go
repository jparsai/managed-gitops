...
var _ = Describe("ApplicationEventLoop Test", func() {
	...
	Context("Handle deployment modified", func() {
		...
		It("Should update existing deployment, instead of creating new.", func() {
			...
			Expect(err).To(BeNil())
			Expect(message).To(Equal("createdNewApp"))

			// ----------------------------------------------------------------------------
			// Verify that database entries are created.
			// ----------------------------------------------------------------------------
			...
			_, _, _, _, err = appEventLoopRunnerActionSecond.applicationEventRunner_handleDeploymentModified(ctx, dbQueries)
			Expect(err).To(BeNil())
		...
		It("Should not deploy application, as request data is not valid.", func() {
			...
			// ----------------------------------------------------------------------------
			// Verify that the database entry is not created
			// ----------------------------------------------------------------------------
		...
		It("Should update existing deployment, if no changes were done in fields.", func() {
			...
			Expect(err).To(BeNil())
			Expect(message).To(Equal("noChangeInApp"))
