...
func (a *applicationEventLoopRunner_Action) applicationEventRunner_handleUpdateDeploymentStatusTick(ctx context.Context,
	...
	gitopsDeployment.Status.Resources, _ = decompressResourceData(applicationState.Resources)
...
func decompressResourceData(resourceData []byte) ([]managedgitopsv1alpha1.ResourceStatus, error) {
	...
	for {
		_, err := io.CopyN(&bufferOut, gzipReader, 1024)
		if err != nil {
			if err == io.EOF {
				break
			}
	...
	err = yaml.Unmarshal(bufferOut.Bytes(), &resourceList)
	if err != nil {
