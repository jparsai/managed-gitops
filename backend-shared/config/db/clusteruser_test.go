...
var _ = Describe("ClusterUser Tests", func() {
	Context("It should execute all DB functions for ClusterUser", func() {
		It("Should execute all ClusterUser Functions", func() {
			...
			Expect(err).To(BeNil())
			Expect(specialClusterUser.Clusteruser_id).To(Equal("cluster-agent-application-sync-user"))
			Expect(specialClusterUser.User_name).To(Equal("cluster-agent-application-sync-user"))
